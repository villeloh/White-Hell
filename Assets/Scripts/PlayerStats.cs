...
public class PlayerStats : MonoBehaviour
{
	...
	public void AddToInv (Item item, string itemName)
	{
        ...
        // This condition ensures that you can always pick up new food items.
        // As a side effect, though, some of the items might have 0 eatValue (when you're already carrying the max amount of food allowed)...
            if ((carriedFood + item.EatValue) > maxCarriedFood)
            {
                item.EatValue = maxCarriedFood - carriedFood;
            }
            carriedFood += item.EatValue;

    }

	// pitäisi poistaa sekä key että value... saa nähdä, toimiiko?
    ...
    */
    ...
    public void EatFoodItem(Item item)
    {
        print("hunger ennen syöntiä:" + hunger); // debug

        // Added a check to ensure that hunger won't go below zero under any circumstances.
        if (hunger >= item.EatValue) {
            hunger -= item.EatValue;
        } else if (hunger < item.EatValue) {
            hunger = 0.0f;
        }

        carriedFood -= item.EatValue; // carriedFoodin ei pitäisi koskaan mennä alle nollan, koska sitä lisätään aina ensin samalla määrällä (kun food item laitetaan inventoryyn)
        inventory.Remove(item);
        item = null; // esineet eivät näemmä katoa syödessä tästä huolimatta, mikä voi olla ongelma...
